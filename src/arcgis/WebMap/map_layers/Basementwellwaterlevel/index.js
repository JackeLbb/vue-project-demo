...
class Basementwellwaterlevel {
    ...
    arrs.map(obj => {
      ...
      if (obj.points && obj.points.length > 0) {
        const symbol = new this.$Gis_api.PictureMarkerSymbol({
          url: '/static/map/icons/basementwellwaterlevel.png',
          width: '30px',
          height: '30px'
        })
        let geometry = new this.$Gis_api.Point(
  ...
  getDatas(_this) {
    GetMapFacilityList({
      facilityCategoryCodes: JSON.stringify(['basementwellwaterlevel'])
    }).then(res => {
      res && res.rows.length > 0 && this.add(res.rows)
    })
  }
}
export default Basementwellwaterlevel
