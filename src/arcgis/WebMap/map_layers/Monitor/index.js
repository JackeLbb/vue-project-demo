...
class Monitor {
    ...
    arrs.map(obj => {
      ...
      if (obj.points && obj.points.length > 0) {
        const symbol = new this.$Gis_api.PictureMarkerSymbol({
          url: '/static/map/icons/monitor.png',
          width: '30px',
          height: '30px'
        })
  ...
  initDatas(_this) {
    return new Promise((resolve, reject) => {
      GetMapFacilityList({
        facilityCategoryCodes: JSON.stringify(['monitor'])
      }).then(res => {
        _this.layercount_progress.push({ layerCode: 'monitor', layerName: '监控', layerCount: res.rowCount, progressCount: res.rows.length })
