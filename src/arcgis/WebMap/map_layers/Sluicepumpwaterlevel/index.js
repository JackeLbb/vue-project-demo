...
class Sluicepumpwaterlevel {
    ...
    arrs.map(obj => {
      ...
      if (obj.points && obj.points.length > 0) {
        const symbol = new this.$Gis_api.PictureMarkerSymbol({
          url: '/static/map/icons/sluicepumpwaterlevel.png',
          width: '30px',
          height: '30px'
        })
  ...
  getDatas(_this) {
    GetMapFacilityList({
      facilityCategoryCodes: JSON.stringify(['sluicepumpwaterlevel'])
    }).then(res => {
      res && res.rows.length > 0 && this.add(res.rows)
    })
  }
}
export default Sluicepumpwaterlevel
